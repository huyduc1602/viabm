<?php
    ...
    $title = langByVn('CHECK LIVE FACEBOOK').' | '.$CMSNT->site('tenweb');
...
<div class="content-i">
                        ...
                        <form class="form-horizontal" role="form" method='post' name='cms_form' novalidate>
                            ...
                            <div id='form_box'>
                                <div class="form-group">
                                    <label class="col-sm-12 control-label" style='text-align:left'><?=langByVn('Nhập Danh Sách UID');?></label>
                                    <div class="col-sm-12">
                                        <textarea style='height:150px;white-space:pre;overflow-wrap:normal;'
                                            class="form-control" id='fb_ids' placeholder="<?=langByVn('Mỗi dòng 1 UID, có thể nhập full định dạng như: UID|PASS|...');?>" value=''></textarea>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="col-sm-12">
    ...
    function check_live_one_nick(fb_id, callback) {
        $.get("<?=BASE_URL('assets/ajaxs/check-live-fb.php?uid=');?>" + fb_id, function(data, status) {
            if (data == 'LIVE') {
